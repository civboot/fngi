...
def gen_zoa_h(path, outPath, brief):
  with open(path, 'rb') as f: p = zoa.Parser(f.read())
  p.parse()
  vals = {nice(k): v for k, v in p.env.vals.items()}
  spor = []
  for k, v in vals.items(): spor.append(f'#define {k:<8}  0x{v:02X}')
  write(
    outPath,
    c_header.format(
      filename=outPath,
...
def build():
  ...
  gen_zoa_h("boot/const.zty", "bin/const.h", "Constants")
  gen_zoa_h("boot/spor.zty", "bin/spor.h", "spor assembly bytecode")
