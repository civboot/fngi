...
@heap .4 FT^ =h2  // h2: {val:2} push 2bytes from stack to heap
  .4 FTML SWP; .2@heap,  // fetch heap and swap {heap, val}
  .2      SR;             // store 2 byte value at heap
  .4 FTML INC2; .2@heap, // fetch heap and increment by 2
  .4 SRML  RET; .2@heap, // store new heap and return

@heap .4 FT^ =h4  // h4: {val:4} push 4bytes from stack to heap
  .4 FTML SWP; .2@heap,  // ft heap
  .4      SR;             // sr 4byte value
  .4 FTML INC4; .2@heap, // ft heap and +4
  .4 SRML  RET; .2@heap, // sr new heap
...
@heap .4 FT^ =_dict // setup for dict
  ...
  .2 LIT RET; @c_dictHeap $h2 // &dict.heap
...
@heap .4 FT^ =c_setRKeyMeta // {mask:U1} mask current key's 8bit meta
  .4 FTML INC4; @c_rKey $h2 // fetch {mask &key.meta}
  .1 FT; // fetch 1 byte {mask meta}
  .1 OR;
  .4 FTML INC4; @c_rKey $h2
  .1 SR RET;

@heap .4 FT^ =locSetup
  // update rKey
  .4 FTML; @c_dictBuf $h2 // dict.buf
  .2 FTML; @c_dictHeap $h2 // dict.heap
  .4 ADD SRML RET; @c_rKey $h2 // rKey=dict.buf + dict.heap
...
@heap .4 FT^ =loc  // rename locFn or something
  ...
  .4 FTML XSL; @heap $h2 @dictSet $h2

  // // set rKey as fn type
  .4 LIT XSL; @IS_FN $h2 @c_setRKeyMeta $h2

  // clear locals by setting localDict.heap=dict.heap (start of localDict.buf)
  .2 SRML ZERO; @c_localOffset $h2   // zero localDict.offset
  .2 SRML ZERO RET; @c_dictLHeap $h2 // zero localDict.heap
...
.4
$loc getHeap     FTML RET; @heap $h2
$loc setHeap     SRML RET; @heap $h2
$loc getTopHeap  FTML RET; @topHeap $h2
$loc setTopHeap  SRML RET; @topHeap $h2
