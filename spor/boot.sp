...
// - Token scanner (D_scan). Tokens are the same for spor and fngi
...
// The following modify the current function's metadata, which is used by the
// compiler to determine how to compile tokens it encounteres.
//
// In addition, MODIFY can be used to change the "current function", allowing
// modificaitons on past functions.
//
// - INSTANT: make an instant function (run immediately by default)
// - ALI: make an always-instant function (always run immediately)
// - PRE: make a prefixed function (run after parsing next by default)


$loc MODIFY   $xsl dictGetR  SRML RET; @c_rKey $h2
$loc INSTANT  LIT @IS_INSTANT $mem_jmpl c_setRKeyFnMeta
$loc SUPER    LIT @IS_SUPER $mem_jmpl c_setRKeyFnMeta
$loc PRE      LIT @IS_PRE $mem_jmpl c_setRKeyFnMeta

$loc c_makeFn LIT @IS_FN $mem_jmpl c_setRKeyMeta
$loc c_makeGlobal LIT @IS_GLOBAL $mem_jmpl c_setRKeyMeta

// Global Variables
$MODIFY heap $c_makeGlobal
$MODIFY heap $c_makeGlobal
$MODIFY topHeap $c_makeGlobal
$MODIFY topMem $c_makeGlobal
$MODIFY err $c_makeGlobal
$MODIFY c_state $c_makeGlobal
$MODIFY testIdx $c_makeGlobal

$MODIFY c_dictBuf $c_makeGlobal
$MODIFY c_dictHeap $c_makeGlobal
$MODIFY c_dictEnd $c_makeGlobal
$MODIFY c_dictLHeap $c_makeGlobal
$MODIFY c_tokenBuf $c_makeGlobal
$MODIFY c_tokenLen $c_makeGlobal

$MODIFY c_rKey $c_makeGlobal
$MODIFY c_localOffset $c_makeGlobal

// These were defined in asm2.sa before loc
$MODIFY c_setRKeyMeta $c_makeFn
$MODIFY h2 $c_makeFn
$MODIFY h4 $c_makeFn
$MODIFY hL4 $c_makeFn
$MODIFY dictSet $c_makeFn
$MODIFY loc $c_makeFn

// TODO: this should be done with a register instead.
$loc assertEq // {a b code}
  #4 $decl_lo code $end_lo
  .4 $setl code
  .4 EQ; $getl code
  $jmpl assert

// Normal way to define a global
// Ex: #4 $c_global foo
$loc declGlobal
  $xsl locSetup
  DUP $xsl getHeap // {sz sz heap}
  SWP $xl  alignSz // {sz heapAligned}
  DUP $xsl dictSet // {sz heapAligned}
  $xsl c_makeGlobal
  ADD $jmpl setHeap

#4 $declGlobal foo
@foo #42 .4 SR^   @foo .4 FT^ #42 $tAssertEq


$loc rValueToMeta .4 INC4; FT RET; // {&value -> meta}

$loc maskRValueMeta // {flags &value} check flags on dict &value meta
  $xsl rValueToMeta
  AND RET;

$loc getg // compile a global variable fetch.
  $xsl dictGetR
  DUP;
  LIT @META_TY_MASK $mem_xsl maskRValueMeta
  $L @IS_GLOBAL
  @E_cNotGlobal $mem_xl assertEq
  // TODO: not done
